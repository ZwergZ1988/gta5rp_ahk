...
add_shpora_to_gui(myGui, coordX, coordY, shpora) {
  offsetY := 0
  offsetY := offsetY + add_text_to_gui(myGui, coordX, coordY, [{text:shpora.title, style: "U"}])

  for index, shporaLine in shpora.lines {
    offsetY := offsetY + add_text_to_gui(myGui, coordX, coordY + offsetY, [{text: shporaLine}])
  }
