...
add_text_to_gui(coordX, coordY, textLines) {
    for index, guiLine in textLines {
        lineCoordinates := coordY+(index-1)*13
...
add_law_chapter_to_gui(coordX, coordY, lawChapter) {
    ...
    add_text_to_gui(coordX, coordY, guiLines)
    ...
    for lawIndex, law in lawChapter[3] {
        ...
        guiLines.Push([[lawColor,law[1]]])
    }    
    add_text_to_gui(coordX, coordY+yOffset, guiLines)

    guiLines := []
    for lawIndex, law in lawChapter[3] {
        guiLines.Push([["cWhite",law[2]]])
    }    
    add_text_to_gui(coordX+50, coordY+yOffset, guiLines)

    guiLines := []
    for lawIndex, law in lawChapter[3] {
        guiLines.Push([["cWhite",law[3]]])
    }    
    add_text_to_gui(coordX+500, coordY+yOffset, guiLines)
}
...
toggle_gui(show_gui) {
    ...
    If show_gui {
        ...
        add_law_chapter_to_gui(lawPositionX,lawPositionY,lc)
        lawPositionY := lawPositionY + 140
        ...
        lc := [""
           ...
           ,[["9.1","Незаконное собирание или распространение сведений о частной жизни лица","до 2 лет (20 м)","Ф"]
            ,["9.2","Незаконное проникновение в жилище","до 6 лет (60 м)","ФР"]]]
        add_law_chapter_to_gui(lawPositionX,lawPositionY,lc)
        lawPositionY := lawPositionY + 47
        ...
        lc := ["Раздел 5. Преступления против общественной безопасности и общественного порядка"
           ...
           ,[["12.1","Терроризм ***","до 30 лет (5 ч)","Ф"]
            ,["12.6","Злостное хулиганство *","до 12 лет (2 ч)","ФР"]
;1) Нарушение два или более раз подряд ст. 20.1 без наказания​
;​2) Следующие деяния, совершенные в общественных местах:​
;2.1) Нецензурные выражения, оскорбительные жесты и приставания к людям, провокация конфликтов. ​
;2.2) Использование любых средств или веществ, которые могут привести к пожару или взрыву;​
;2.3) Действия, влекущие за собой опасные последствия для окружающих и самого нарушителя;​
;2.4) Агитационной деятельности без разрешения руководства учреждения;​
;2.5) Проход в служебные помещения и на рабочие места сотрудников государственного учреждения.​
;2.6) Ношение холодного оружия в том числе спорт.инветаря и инструментов (кроме случаев самообороны).
        ...
        lc := [""
           ...
           ,[["15.1","Превышение должностных полномочий с существенным нарушеним прав **","24 года (4 ч)","Ф"]
            ,["15.1.1","Злоупотребление должностными полномочиями с существенным нарушеним прав *","12 лет (2 ч)","Ф"]
            ,["15.2","Умышленное неисполнение сотрудником государственных структур законного приказа начальника *","6 лет (60 м)","Ф"]
            ...
            ,["15.6","Халатность с существенным нарушеним прав **","12 лет (2 ч)","ФР"]
            ,["15.6.1","Неисполнение или ненадлежащее исполнение должностным лицом своих обязанностей *","4 года (40 м)","Р"]]]
        add_law_chapter_to_gui(lawPositionX,lawPositionY,lc)
        lawPositionY := lawPositionY + 125

        lc := [""
            ,"Глава 16. Преступления против правосудия"
           ,[["16.14","Уклонение от расследования, задержания и суда **","18 лет (3 ч)","Ф"]]]
        add_law_chapter_to_gui(lawPositionX,lawPositionY,lc)
        lawPositionY := lawPositionY + 32

        lc := [""
            ,"Глава 17. Преступления против порядка управления"
           ,[["17.1","Посягательство на жизнь Госника ***","24 года (4 ч)","ФР"]
            ,["17.2","Применение насилия, не опасного для жизни или здоровья, либо угроза применения насилия в отношении госника *","18 лет (3 ч)","ФР"]
            ,["17.3","Оскорбление госника. *","4 года(40 м)","Р"]
;Оскорбление заключается в негативной оценке личности либо внешности человека, его качеств, поведения, причем в форме,
;противоречащей установленным правилам поведения и требованиям общечеловеческой морали.
            ,["17.3.1","Провокация государственного служащего при исполнении, на дороге путем многократного создания аварийной ситуации.","2 года (20 м)","Р"]
            ,["17.3.2","Провокация государственного служащего при исполнении путем помехи выполнения им своих служебных обязанностей. *","4 года (40 м)","Р"]
            ,["17.4","Использование подделанного удостоверения или иного официального документа, предоставляющего определённые полномочия **","12 лет (2 ч)","Ф"]
            ,["17.5","Самоуправство **","12 лет (2 ч)","ФР"]
            ,["17.6","Неповиновение законному требованию либо распоряжению правомочного лица *","4 года (40 м)","ФР"]
            ,["17.7","Отказ от оплаты штрафа выписанного сотрудником правоохранительных органов *","4 года (40 м)","ФР"]]]
        add_law_chapter_to_gui(lawPositionX,lawPositionY,lc)
        lawPositionY := lawPositionY + 100
        ...
        add_text_to_gui(0, 730, mirandaLines)
...
;►Нахождение в общественном месте без верхней одежды;​ ►Оскорбительное приставание к гражданам;​
;►Нецензурные жесты;​ ►Действия несущие угрозу обществу (размахивание кулаками и другими орудиями);​
;►Драка в общественных местах ►Попрошайничество на территории гос. учреждения или в общ. местах;​
;►Праздношатание, т.е. пребывание на тер. гос. учреждения без определенной цели или без законного повода.
;5 000 $
;20.2(А)
;Умышленное повреждение чужому имуществу без знач. ущерба
;1 000 $
;20.3(А)
;Стрельба без причины в городской черте
;5 000 $
;20.5(А)
;Употребление спиртных напитков и наркотических веществ в общественном месте, а также нахождение в общественном месте в состоянии алкогольного и наркотического опьянения.
;4 000 $
;20.6(А)
;Оскорбление, то есть умышленное унижение чести и достоинства личности гражданского лица​
;5 000 $
;21.3(А)
;Воспрепятсвование работе EMS
;5 000 $
;21.4(А)
;Умышленное повреждение чужому имуществу
;3 000 $
;21.5(А)
;Тайное хищение чужого имущества
;5 000 $
;22.1(А)
;Курение в гос. учереждениях
;3.5 - 5 000 $
;22.2(А)
;Разведение костров в общественном месте
;3 - 5 000 $
;22.3(А)
;Использование колонок в гос. учереждениях
;3 - 4 000 $
;23.1(А)
;Браконьерство рыбалка (без лицензии до 30 кг и только на пирсе)
;+ по 500$ за 1кг привышения по 4.2.1 закона о рыбалке (?????)
;3 - 15 000 $
;23.2(А)
;Браконьерство охота (можно до 15 кг без лицензии)
;5 - 20 000 $
