...
build_law_search_gui() {
    ...
    inputObj := InputBox("Введите слово или номер статьи для поиска по УК", "Поиск по УК", "W350 H90")
            ...
            for zakon in glava.zakony {
                for stro4ka in zakon.stro4ki {
                    if (InStr(zakon.nomer, inputObj.Value, "Locale") or InStr(stro4ka, inputObj.Value, "Locale")) {
                        foundZakony.push({razdel: razdel.name, glava: glava.name, zakon: zakon})
                        break
                    }
                }
            }
